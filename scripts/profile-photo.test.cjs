...
const { getPhotoCrop, uploadProfilePhoto } = require('../src/lib/profilePhoto.ts');
const { normalizePortfolioData } = require('../src/lib/portfolio.ts');
const { EXAMPLE_PORTFOLIO } = require('../src/lib/example.ts');
const url = 'https://test.public.blob.vercel-storage.com/profile-photos/photo.jpg';
...
function setup() {
  process.env.BLOB_READ_WRITE_TOKEN = 'test-token-not-real';
  released = false;
...
test('upload validates, re-encodes, strips metadata, stores the image and returns a public URL', async () => {
  setup();
  ...
  assert.equal(res.code, 201); assert.equal(res.body.url, url); assert.ok(updated && released);
});
test('rejects wrong origins, sessions, non-images and oversized input before storing', async () => {
  ...
  for (const [patch, code] of [
    [{ method: 'GET' }, 405], [{ headers: { origin: 'https://other.test', host: 'repofolio.test' } }, 403],
...
test('reports setup, quota, expired session and database errors without uploading', async () => {
  setup(); delete process.env.BLOB_READ_WRITE_TOKEN;
...
test('client refreshes expired sessions and propagates upload failure', async () => {
  ...
  try {
    global.fetch = async endpoint => endpoint === '/api/session'
      ...
      : ++uploads === 1 ? { status: 401 } : { ok: true, json: async () => ({ url }) };
    assert.equal(await uploadProfilePhoto('/9j/AA=='), url); assert.equal(uploads, 2);
...
test('normalization preserves uploaded and original GitHub photos for snapshots and drafts', () => {
  const portfolio = { ...EXAMPLE_PORTFOLIO, avatarUrl: url, githubAvatarUrl: 'https://avatars.githubusercontent.com/u/123' };
  const restored = normalizePortfolioData(JSON.parse(JSON.stringify(portfolio)));
  assert.equal(restored.avatarUrl, url); assert.equal(restored.githubAvatarUrl, portfolio.githubAvatarUrl);
