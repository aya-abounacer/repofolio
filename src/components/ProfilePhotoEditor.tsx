...
import { deleteProfilePhoto, getPhotoCrop, uploadProfilePhoto, type PhotoCrop } from '../lib/profilePhoto'
import { photoReference } from '../lib/photoReference'
...
export function ProfilePhotoEditor({ avatarUrl, githubAvatarUrl, onChange, onBusyChange }: Props) {
  ...
  const currentAvatar = useRef(avatarUrl)
  currentAvatar.current = avatarUrl
  ...
  async function applyPhoto() {
    ...
    try {
      ...
      setUploadPreview(canvas.current.toDataURL('image/jpeg', 0.75))
  ...
  async function deleteCurrentPhoto() {
    if (uploading || deleting || !window.confirm('Delete this uploaded photo? It will disappear from any portfolios using it.')) return
    setDeleting(true)
    onBusyChange(true)
    setError('')
    try {
      await deleteProfilePhoto(currentAvatar.current)
      setPhoto(null)
      onChange(githubAvatarUrl)
    } catch (caught) {
      setError(caught instanceof Error ? caught.message : 'Could not delete this photo. Try again.')
    } finally {
      setDeleting(false)
      onBusyChange(false)
    }
  ...
  return <div className="space-y-3">
    ...
    <div className="flex items-center gap-3">
      <img src={uploadPreview || pendingPreview || avatarUrl || githubAvatarUrl} alt={uploadPreview ? 'Photo being uploaded' : pendingPreview ? 'Selected photo being prepared' : 'Current profile photo'} className="h-16 w-16 shrink-0 rounded-xl border border-white/15 object-cover" />
      <div className="flex flex-wrap gap-2">
        <button type="button" className="studio-mini-button" disabled={uploading || deleting || loading} onClick={() => input.current?.click()}>{loading ? 'Opening…' : 'Upload photo'}</button>
        {photoReference(avatarUrl) ? <button type="button" className="studio-mini-button" disabled={uploading || deleting || loading} onClick={() => void deleteCurrentPhoto()}>{deleting ? 'Deleting…' : 'Delete photo'}</button> : null}
      </div>
    ...
    <PrivacyControls kind="photos" onRemoved={(item) => {
      const reference = photoReference(currentAvatar.current)
      if (reference?.id === item.id || (item.legacy_url && reference?.legacy === item.legacy_url)) onChange(githubAvatarUrl)
    }} />
